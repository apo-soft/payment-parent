...
public class QRcodeServlet extends HttpServlet {
	...
	private volatile String defaultErrorImgPath = "/error.";
	...
	/**
	 * 入参格式：http://www.aposoft.cn/payment/qrcode?width=150&height=150&content={使用http urlParamEncoding编码的内容}[&type=[png|jpg]]
	 * width:必填 ，范围：最小100，最大：2000；<br/>
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(checkInput(width,height,content,type)){
			byte[] bytes;
			try {
				bytes = QRCodeUtil.getQrCodeImageBytes(content, type, Integer.parseInt(width), Integer.parseInt(height), defaultContentEncoding);
