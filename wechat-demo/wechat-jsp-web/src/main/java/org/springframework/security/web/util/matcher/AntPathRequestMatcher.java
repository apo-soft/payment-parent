...
public final class AntPathRequestMatcher implements RequestMatcher {
	...
	public AntPathRequestMatcher(String pattern) {
		this(pattern, null);
		logger.info("simple constructor is called:" + pattern);
	}
	...
	public AntPathRequestMatcher(String pattern, String httpMethod) {
		...
		logger.info("httpMethod constructor is called:" + pattern + "," + httpMethod);
	}
	...
	public AntPathRequestMatcher(String pattern, String httpMethod, boolean caseSensitive) {
		...
		if (pattern.equals(MATCH_ALL) || pattern.equals("**")) {
			...
			logger.info(seq + " creation path match pattern:" + pattern);
			...
			if (pattern.endsWith(MATCH_ALL)
				...
				logger.info(seq + " create SubpathMatcherr:" + pattern.substring(0, pattern.length() - 3));
				matcher = new SubpathMatcher(pattern.substring(0, pattern.length() - 3));
			} else {
				logger.info(seq + " create SpringAntMatcher:" + pattern);
	...
	public boolean matches(HttpServletRequest request) {
		...
		if (httpMethod != null && StringUtils.hasText(request.getMethod())
			...
			logger.debug("Matcher.match end with result because of httpMethod not match false");
			return false;
		}
		...
		if (pattern.equals(MATCH_ALL)) {
			...
			logger.debug("Matcher.match endend with result true");
			return true;
		}
		...
		logger.info(seq + "matcher:" + matcher.toString() + "path:" + url + " to match.");
	...
	/**
	 * Provides a save way of obtaining the HttpMethod from a String. If the
	 * method is invalid, returns null.
	 *
	 * @param method
	...
	private static class SpringAntMatcher implements Matcher {
		...
		public boolean matches(String path) {
			logger.info("match:pn:" + pattern + ",ph:" + path);
	...
	private static class SubpathMatcher implements Matcher {
		...
		public boolean matches(String path) {
			logger.info("subpath Matcher:" + subpath);
